...
if(isset($_POST['submit'])){
	...
	if($category=="doctors"){
		$query=mysqli_prepare($link,"insert into id17137158_pmsproject.doctormessages (docID,message,adminID) values (?,?,?)");
	}else{
		$query=mysqli_prepare($link,"insert into id17137158_pmsproject.patientmessages (patientID,message,adminID) values (?,?,?)");
	}
...
<script>
	function checkemailAvailability() {
		...
		jQuery.ajax({
			...
			data:'email='+$("#email").val()+'&cat='+$("#category").val(),
			type: "POST",
				...
				<tbody>
					<form name="query" method="post">
						<tr>
							...
							<td>
								<select id="category" name="category" class="form-control" onChange="checkemailAvailability()">
						...
						<tr>
							<th>Name</th>
							<td><?php ?><span id="email-availability-status2"></span></td>
						</tr>
						<tr>
							<th>Title</th>
							<td><?php ?><input type="text" id="title" name="title" class="form-control" placeholder="Title" value=""></td>
						</tr>
						...
						<tr>
							...
							<td>
								<div class="text-right">
									...
									<button type="submit" name="submit" id="submit" class="btn btn-success" name="send">Send Message</button>
								</div>
							</td>
						</tr>
					</form>												
