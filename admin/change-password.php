...
if(isset($_POST['submit'])){
	...
	if ( empty( $password_err ) && empty( $npassword_err ) && empty( $cpassword_err ) ){
		$sql = mysqli_query( $link, "SELECT password FROM id17137158_pmsproject.admins WHERE id = '".$_SESSION['id']."'");
		$row = mysqli_fetch_array( $sql );
		$hashed_password = $row['password'];
		if ( password_verify( $password, $hashed_password ) ){
			$sql = mysqli_prepare( $link, "UPDATE id17137158_pmsproject.admins set password = ? WHERE id = '".$_SESSION['id']."'");
