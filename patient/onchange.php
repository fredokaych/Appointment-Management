...
if ( !empty( $_POST[ 'msgtype' ] ) ) {
    if ( $_POST[ 'msgtype' ] == 'unread' ) {
        $sqlstr = "select * from id17137158_pmsproject.patientmessages where patientID = ".$id." and IsRead = 0 order by id desc";
    } elseif ( $_POST[ 'msgtype' ] == 'read' ) {
        $sqlstr = "select * from id17137158_pmsproject.patientmessages where patientID = ".$id." and IsRead = 1 order by id desc";
    } else {
        $sqlstr = "select * from id17137158_pmsproject.patientmessages where patientID = ".$id." order by id desc";
    }
